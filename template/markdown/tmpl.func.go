...
//获取短文字
func isNull() func(*model.TmplCol) string {
	return func(row *model.TmplCol) string {
		return IsNull[row.IsNull]
	}
}

func isMDNull() func(*model.TmplCol) string {
	return func(row *model.TmplCol) string {
...
//通过正则表达式，转换正确的数据库类型
func defValue(tp string) func(*model.TmplCol) string {
...
func getPKS(t *model.TmplTable) []string {
	return t.GetPKS()
}

func getSEQTag(tp string) func(r *model.TmplCol) string {
	switch tp {
	case MYSQL:
		return func(r *model.TmplCol) string {
			if isCons(r.Con, "seq") {
	...
	return func(r *model.TmplCol) string { return "" }
}

func getSEQValue(tp string) func(r *model.TmplTable) string {
	switch tp {
	case MYSQL:
		return func(r *model.TmplTable) string {
	...
	return func(r *model.TmplTable) string { return "" }
}

func getSEQ(tp string) func(r *model.TmplTable) bool {
	switch tp {
	case MYSQL:
		return func(r *model.TmplTable) bool {
			for _, r := range r.RawRows {
				if isCons(r.Con, "seq") {
					return true
				}
	...
	return func(r *model.TmplTable) bool { return false }
}
...
func sortByKw(kw string) func(rows TableColumn) []*model.TmplCol {
	return func(rows TableColumn) []*model.TmplCol {
...
func getDBIndex(tp string) func(r *model.TmplTable) string {
	switch tp {
	case MYSQL:
		return func(r *model.TmplTable) string {
			indexs := r.GetIndexs()
...
func decodeRows(tp, deftp string) func(row []*model.TmplCol) []*model.TmplCol {
	return func(row []*model.TmplCol) []*model.TmplCol {
		list := make([]*model.TmplCol, 0, 1)
		for _, r := range row {
			if isCons(r.Con, tp) {
				list = append(list, r)
			}
...
func HasRow(row []*model.TmplCol, tp ...string) bool {
	return len(getRows(tp...)(row)) > 0
}

func getRows(tp ...string) func(row []*model.TmplCol) []*model.TmplCol {
	return func(row []*model.TmplCol) []*model.TmplCol {
		list := make([]*model.TmplCol, 0, 1)
		for _, r := range row {
		NEXT:
...
func replace(tp string) func(row *model.TmplCol) string {
	return func(row *model.TmplCol) string {
...
func getDicName(keys ...string) func(con string, subcon string, tb *model.TmplTable) string {
	return func(con string, subcon string, tb *model.TmplTable) string {
		tp := subcon
...
func getCascadeChildrenName(tp, tkey string, keys ...string) func(name string, t *model.TmplTable) string {
	return func(name string, t *model.TmplTable) string {
...
func getCascadeParentName(tp, tkey string, keys ...string) func(con string, t *model.TmplTable) string {
	return func(con string, t *model.TmplTable) string {
...
func getChildrenGroup(tp string) func(name string, t *model.TmplTable) []*model.TmplCol {
	return func(name string, t *model.TmplTable) []*model.TmplCol {
		r := []*model.TmplCol{}
		var wg sync.WaitGroup
		var lock sync.Mutex
		for _, v := range t.Rows {
			wg.Add(1)
			go func(tp string, v *model.TmplCol, t *model.TmplTable) {
				defer wg.Done()
...
func hasKW(tp ...string) func(t *model.TmplTable) bool {
	return func(t *model.TmplTable) bool {
		ext := map[string]bool{}
